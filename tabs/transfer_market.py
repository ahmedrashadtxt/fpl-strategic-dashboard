...
def render_transfer_market_tab(conn, current_gw):
  ...
  if market_df.empty:
    st.info(
        "No players found matching your transfer search or squad filter"
        " criteria."
    )
  else:
    ...
    with col_in:
      ...
      if heating_df.empty:
        ...
        for _, row in heating_df.iterrows():
          ...
          change_tag = (
              ("Rising", "green") if row["Total_Change"] > 0 else ("Flat", "gray")
          )
          render_list_card(
    ...
    with col_out:
      ...
      if cooling_df.empty:
        ...
        for _, row in cooling_df.iterrows():
          progress = min(
              (abs(min(0, row["Net_Transfers"])) / THRESHOLD) * 100, 100
          )
          change_tag = (
              ("Falling", "red") if row["Total_Change"] < 0 else ("Flat", "gray")
          )
          render_list_card(
              f"{row['Player']} · {row['Team']}",
