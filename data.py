...
def get_global_gameweek_info(conn):
  """
  Fetches current, ongoing, and next gameweek metadata directly from the official FPL API events table.
  """
...
def calculate_projected_points(player_row, fix_info, current_gw_num, hist_baselines_df=None):
  ...
  Employs expected minutes scaling, sample-size shrinkage on underlying rates,
  and Empirical Bayesian blending between historical and current baselines.
  """
  ...
  # ── 1. Expected Minutes Calculation ───────────────────────────────────────
  ...
  if is_target:
    ...
    # Starter Expected Minutes Baseline for established starters returning from injury
  ...
  # Probability of reaching the 60-minute clean sheet and appearance threshold
  p_60_mins = max(0.0, min(1.0, (exp_mins - 25.0) / 45.0)) if exp_mins >= 25.0 else 0.0
  mins_ratio = exp_mins / 90.0

  # ── 2. Position Baselines & Sample Shrinkage ──────────────────────────────
  ...
  # Bayesian shrinkage on small-sample current season rates
  ...
  # ── 3. Fixture & Underlying Attack/Defense Scalers ─────────────────────────
  ...
  if pos in ("GKP", "DEF"):
    ...
    # Expected attacking returns scaled by expected minutes
  ...
  # ── 5. Status & Availability Multiplier ───────────────────────────────────
  status = str(player_row.get("Status", player_row.get("status", "a"))).lower()
...
def create_price_predictions_table(conn):
  """Creates the table for expected nightly price changes."""
  cursor = conn.cursor()
  cursor.execute("""
...
def calculate_price_change_predictions(conn) -> pd.DataFrame:
  """
  Estimates nightly price rise/fall progress (-100% to +100%)
  based on ownership volume thresholds, flags, and net transfers.
  """
  ...
  with conn:
    pred_df.to_sql("player_price_predictions", conn, if_exists="replace", index=False)
...
def get_price_prediction_map(_conn) -> dict:
  """In-memory cache mapping element_id to expected price progress."""
  query = """
