...
def build_player_tooltip(p: pd.Series, horizon_len: int = 1) -> str:
    ...
    cost = p.get("Cost", 0.0)
    cost_str = f"┬ú{fmt_num(cost, '.1f')}m" if cost else "ΓÇö"
    ...
    if status != "a" and news and news != "None":
        ...
        news_row = f'<div class="tt-row tt-news"><span>ΓÜáÅ {clean_news}</span></div>'

    transfer_badge_row = ""
    if bool(p.get("is_transfer_in") is True):
        transfer_badge_row = '<div class="tt-row" style="color:#34d399; font-weight:700;"><span>ƒƒó Proposed Sign</span></div>'
    elif bool(p.get("is_transfer_out") is True):
        transfer_badge_row = '<div class="tt-row" style="color:#f87171; font-weight:700;"><span>ƒö Proposed Sale</span></div>'
...
def apply_market_projection_with_movement(
    ...
    if abs(diff) >= 0.30:
        dis_item = {
            ...
            "Verdict": "Market Bullish ƒôê" if diff > 0 else "Market Bearish ƒôë",
        }
...
def render_transfer_pitch_component(
    ...
    for pos in pos_rows:
        ...
        for _, p in row_players.iterrows():
            ...
            cost = p.get("Cost", 0.0)
            cost_str = f"┬ú{fmt_num(cost, '.1f')}m" if cost else ""
    ...
    if bench_df is not None and not bench_df.empty:
        ...
        for idx, (_, b) in enumerate(bench_df.iterrows()):
            ...
            b_cost_str = f"┬ú{fmt_num(b_cost, '.1f')}m" if b_cost else ""
            b_tag = ""
            if is_b_in:
...
def render_transfer_analyzer_tab(conn, events_df, current_gw):
    ...
    if not mgr_to_use:
        st.info("ƒæå Enter your FPL ID in the top search bar to load your squad.")
        return
    ...
    st.markdown("### ΓÜÖÅ Optimization Horizon & Controls")
    c1, c2, c3, c4 = st.columns([1.5, 1.1, 1.1, 1.8])
    with c1:
        horizon_gws = st.selectbox(
            "Evaluation Horizon",
            options=[1, 2, 3, 5],
            format_func=lambda x: f"Next {x} Gameweek{'s' if x > 1 else ''} (GW{next_gw}ΓÇôGW{next_gw + x - 1})",
            index=1,
        )
    with c2:
    ...
    with col_tgl1:
        pitch_view = st.toggle("ƒÅƒÅ **Pitch View**", value=True, key="transfer_pitch_toggle")
    with col_tgl2:
        enable_betting = st.toggle("ƒôè **Betting Market xG**", value=True, key="transfer_betting_toggle")
    with col_tgl3:
        market_weight = 0.35
        if enable_betting:
    ...
    with col_tgl4:
        ...
        if enable_betting:
            factor_movement = st.checkbox("ΓÜí Movement", value=True, key="transfer_factor_movement")
    ...
    locked_players = st.multiselect(
        "ƒöÆ Lock Key Players (Will NOT be transferred out)",
    ...
    m3.metric("Remaining In Bank", f"┬ú{(bank_balance - sum(s['cost_diff'] for s in swaps)):.1f}m")
    m4.metric("Moves Executed", f"{len(swaps)} of {total_allowed_transfers}")

    st.markdown("### ƒöä Recommended Transfer Moves")
    if not swaps:
        st.success("Γ£à Your current squad is optimal for this horizon. No transfer yields higher starting points within your budget.")
    else:
        for s in swaps:
            ...
            with c_out:
                st.markdown(
                    ...
                    <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; padding: 8px 12px;">
                        <span style="font-size: 0.72rem; font-weight: 800; color: #f87171;">ƒö TRANSFER OUT</span><br>
                        <strong>{s['out']['Player']}</strong> ({s['out']['Team']}) ┬ ┬ú{s['out']['Cost']:.1f}m<br>
            ...
            with c_in:
                st.markdown(
                    ...
                    <div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: 8px; padding: 8px 12px;">
                        <span style="font-size: 0.72rem; font-weight: 800; color: #4ade80;">ƒƒó TRANSFER IN</span><br>
                        <strong>{s['in']['Player']}</strong> ({s['in']['Team']}) ┬ ┬ú{s['in']['Cost']:.1f}m<br>
    ...
    st.markdown("### ΓÜûÅ Squad Visual Comparison (Current vs Transfer)")
    ...
    with col_left:
        st.markdown(
            ...
            <div style="height: 28px; display: flex; align-items: center; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                <span style="font-size: 0.92rem; font-weight: 700; color: #f8fafc;">ƒæñ Current Squad ({horizon_gws}-GW Run)</span>
        ...
        if pitch_view:
            ...
            for idx, (_, row) in enumerate(base_xi.iterrows()):
                ...
                render_list_card(
                    ...
                    tags,
                    f'<span>Horizon xP</span> <strong>{fmt_num(row["Horizon_xP"], ".1f")}</strong> ┬ <span>Cost</span> ┬ú{fmt_num(row["Cost"], ".1f")}',
    ...
    with col_right:
        st.markdown(
            f"""
            <div style="height: 28px; display: flex; align-items: center; margin-bottom: 6px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                <span style="font-size: 0.92rem; font-weight: 700; color: #f8fafc;">ƒöä Transfer Squad ({horizon_gws}-GW Run)</span>
    ...
    st.markdown("### ƒôï Multi-Gameweek Performance Ledger")
